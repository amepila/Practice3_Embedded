...
StateReadI2C_Type stateAddress(StateReadI2C_Type data){
	...
	if(getUART1_mailBox() != CR){
		/**Sends to the PCA the received data in the mailbox*/
		UART_putChar(UART_1, getUART1_mailBox());
	}
	if((counter1 > 1) && (counter1 < 8)){
		dataState1.inputAddress[counter1-2] = getUART1_mailBox();
	}
	counter1++;
...
StateReadI2C_Type stateLenght(StateReadI2C_Type data){
	...
	dataState2.inputLenght[counter2] = getUART1_mailBox();
	counter2++;
...
StateReadI2C_Type stateData(StateReadI2C_Type data){
	...
	for(counterChar = 0; counterChar < data.realLenght; counterChar++){
		UART_putChar(UART_1,readMemory((data.realAddress + counterChar)));
		E2PROMdelay(65000);
	}
...
StateWriteI2C_Type stateAddressWrite(StateWriteI2C_Type data){
	...
	if((counterWrite1 > 1) && (counterWrite1 < 8)){
		dataWrite1.inputAddress[counterWrite1 - 2] = getUART1_mailBox();
	}
	counterWrite1++;
...
StateWriteI2C_Type stateDataWrite(StateWriteI2C_Type data){
	...
	dataWrite2.inputData[counterWrite2] = getUART1_mailBox();
	counterWrite2++;
...
StateSetHour_Type stateSetTime(StateSetHour_Type data){
	...
	if(getUART1_mailBox() == CR){
		...
		if(HOUR == flagType){
			...
			UART_putChar(UART_1, ASCII_DOUBLEPOINT);
		}
		if(MINUTES == flagType){
...
StateSetDate_Type stateSetCalendar(StateSetDate_Type data){
	...
	if(getUART1_mailBox() == CR){
		...
		clearFIFO_1();
		clearUART1_mailbox();
		UART_putChar(UART_1, ASCII_DIAG);
...
*/
		...
		if(FALSE == data.flagBlock){
			...
			UART_putChar(UART_1, time[0]);
			UART_putChar(UART_1, time[1]);

			UART_putChar(UART_1,ASCII_DOUBLEPOINT);
			UART_putChar(UART_1, time[2]);
			UART_putChar(UART_1, time[3]);

			UART_putChar(UART_1,ASCII_DOUBLEPOINT);
			UART_putChar(UART_1, time[4]);
			UART_putChar(UART_1, time[5]);
...
StateEco_Type stateTransmitEco(StateEco_Type data){
	...
	if((fifo.data != ESC) && (counter < 50)){
		/**Sends to the PCA the received data in the mailbox*/
		UART_putChar(UART_1, fifo.data[counter]);
...
States_MenuType stateMenu(Time_Type realTime){
	...
	if(FALSE == flagUART1){flagUART1 = menu_Main();}
	...
	if(getUART1_flag()){
		...
		if(getUART1_mailBox() == CR){
			...
			fifoMenu.stateFIFO = clearFIFO_1();
		}
...
States_MenuType stateFormat(Time_Type realTime){
	...
	if(FALSE == flagUART1){
		flagUART1 = menu_FormatHour(phase);
	}
